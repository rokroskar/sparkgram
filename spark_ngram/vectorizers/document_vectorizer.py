"""
Module to process corpora using Spark
"""

from sklearn.feature_extraction.text import CountVectorizer
import re
import sys, glob, os
from lawecon.BloombergCaseExtractor import BloombergCaseExtractor
...
def ngram_hash_frequency(text, ngram_range=[1,1], stop_words = None, tokenizer = alpha_tokenizer,
						 feature_max = 2**32) :
	"""
	Count the frequency of ngrams appearing in document ``text``
...
class SparkDocumentVectorizer(object) :
	...
	*feature_max*: maximum number of expected features. This sets the size of the
	...
	def __init__(self, sc, doclist,
		     ngram_range = [1,1], stop_words = None, nmin = None, nmax = None, num_partitions = None, features_max = 2**32) :
		self._sc = sc
	...
	def apply_filter(self, nmin = None, nmax = None) :
		...
		# docvec_rdd and vocab_rdd are both derived from ngram_rdd, so force reevaluation
	...
	def ngram_rdd(self) :
		...
		if self._ngram_rdd is None :
			self._ngram_rdd = \
			    self.doc_rdd.mapValues(lambda x: ngram_hash_frequency(x, ngram_range, stop_words, features_max))
	...
	def docvec_rdd(self) :
		...
		if self._docvec_rdd is None :
			...
			self._docvec_rdd = self.ngram_rdd.mapValues(lambda x: SparseVector(features_max,[(y[0][1],y[1]) for y in x]))
